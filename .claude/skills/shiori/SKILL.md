...
5. **図に `min-width` を付けて横スクロールさせない。** 過去に `svg{min-width:560px}` を
   `overflow-x:auto` の枠に入れた版があり、スマホでは図の右側が**切れて見えるだけ**だった
   （スワイプできる手がかりが無い）。図は HTML/CSS の棒グラフで組み、どの幅でも収める。
   320・360・390・430・768・1280px で「ページも、各図の枠も」はみ出さないことを確かめる。
6. **`table-layout:fixed` の列幅は先頭行でしか決まらない。** 比較表の項目名列の幅を
   本文行（`tbody th`）に書いても効かず、見出し行の空セルで均等割りになって
   「客室風／呂」のように語の途中で折れた。幅は `thead th:first-child` に書く。
7. **カバー写真は原寸を直リンクしない。** Wikimedia の原寸（2.8MB）は重いうえに
   レート制限（429）で落ちることがある。`thumb.wikimedia.org/.../1280px-…` と
   `1920px-…` を `srcset` で渡す（1280px 版は約229KB）。先読み `<link rel=preload>`
   には `fetchpriority="high"` も付ける（無いと Lighthouse の LCP 検出で落ちる）。
   写真を使うときは作者・ライセンスを Commons の API で確認してから載せる。

## 触ってはいけない / 壊しやすい部分

- **5案の色は `[data-p="a"〜"e"]` が `--p` に配る。** タイル・比較表・料金図・詳細が
  全部この1変数で色を取っているので、案の色を変えるときは `--pa`〜`--pe` だけを触る。
- **料金図の棒の位置は「総額 ÷ 350,000」の百分率。** 総額は「宿2泊＋36,300円」。
  宿の料金を直したら、タイルのメーター・料金図・詳細の「旅の総額」の3箇所を揃えて直す。
